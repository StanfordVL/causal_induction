...
#                 print(np.mean(1*self.correct))
            ...
            if done:
                with open(self.filename + "_S" + str(self.seen) + "_"+str(self.structure)+\
                          "_H"+str(self.horizon)+"_N"+str(self.num)+"_T"+str(self.current_cs)+".txt", "a") as f:
                    f.write(str(self.eprew) + "\n")
                with open(self.filename + "_S" + str(self.seen) + "_"+str(self.structure)+\
                          "_H"+str(self.horizon)+"_N"+str(self.num)+"_T"+str(self.current_cs)+"successrate.txt", "a") as f:
...
#                 perm = np.random.permutation(self.num)
        ...
        obs = self._get_obs()
        # print(obs.shape)
        # im = obs[:-(self.num*self.num)]
        # im = im.reshape(32,32,6)
        # cv2.imwrite('g.png', im[:,:,3:]*255.0)
        # cv2.imwrite('s0.png', im[:,:,:3]*255.0)
        return obs

    def compute_reward(self, obs, info=None):
        im = obs[:-(self.num*self.num)]
        im = im.reshape(32,32,6)
        rew =-1 * np.sqrt(((im[:,:,:3] - im[:,:,3:])**2).sum())
        # cv2.imwrite('s'+str(self.steps)+'_'+str(rew)+'.png', im[:,:,:3]*255.0)
        return rew
...
#             cv2.imwrite("test.png", (255*im).astype(np.uint8))
        ...
        self.sim.model.light_active[:] = light
        o = self.sim.render(width=32,height=32,camera_name="birdview") / 255.0

        if self.gc:
            self.sim.model.light_active[:] = self.goal
            g = self.sim.render(width=32,height=32,camera_name="birdview") / 255.0
            o = np.concatenate([o, g], 2)
        o = o.flatten()
